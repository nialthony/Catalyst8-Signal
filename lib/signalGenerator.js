...
async function fetchOHLCV(symbol, timeframe, limit = 120, options = {}) {
  ...
  try {
    return await fetchFromBinanceFutures(normalizedSymbol, timeframe, limit);
  } catch {
    try {
...
async function fetchFuturesContext(symbol, timeframe) {
  ...
  try {
    const res = await fetch(`https://fapi.binance.com/fapi/v1/premiumIndex?symbol=${normalizedSymbol}`, { signal: AbortSignal.timeout(10000) });
    if (res.ok) premium = await res.json();
  } catch {}

  try {
    const res = await fetch(`https://fapi.binance.com/futures/data/openInterestHist?symbol=${normalizedSymbol}&period=${period}&limit=12`, { signal: AbortSignal.timeout(10000) });
    if (res.ok) openInterestHist = await res.json();
  } catch {}

  try {
    const res = await fetch(`https://fapi.binance.com/futures/data/globalLongShortAccountRatio?symbol=${normalizedSymbol}&period=${period}&limit=12`, { signal: AbortSignal.timeout(10000) });
...
function buildCoinKeywords(symbol, options = {}) {
  const base = symbolBase(symbol).toLowerCase();
  const mapKeywords = SYMBOL_MAP[symbol]?.keywords || [];
  const fromName = String(options?.coinName || '')
    .toLowerCase()
    .split(/[^a-z0-9]+/)
    .filter((x) => x && x.length > 2);
  const fromSymbol = String(options?.coinSymbol || '').toLowerCase();
...
async function fetchCatalystWatch(symbol, options = {}) {
  const normalizedSymbol = normalizeTradingSymbol(symbol);
  const base = symbolBase(normalizedSymbol).toLowerCase();
  const keywords = buildCoinKeywords(normalizedSymbol, options);
