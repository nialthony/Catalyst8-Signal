...
function getCoinGeckoClient() {
  ...
  const proKey = COINGECKO_PRO_API_KEY || (preferredEnv === 'pro' ? COINGECKO_API_KEY : '');
  const demoKey = COINGECKO_DEMO_API_KEY || (preferredEnv === 'demo' ? COINGECKO_API_KEY : '');

  if (!proKey && !demoKey) {
    throw new Error('Missing CoinGecko API key. Set COINGECKO_API_KEY or COINGECKO_DEMO_API_KEY/COINGECKO_PRO_API_KEY.');
  }
  ...
  coingeckoClient = new CoinGeckoSDK({
    environment,
